...
protocol CropperViewDelegate {
    @objc optional func onCroppedImage(image:UIImage)
}
...
final class CropperViewController: UIViewController {
    ...
    weak var delegate: CropperViewDelegate?
    // MARK: -- Actions
    ...
    @IBAction func cropImageAction(_ sender: AnyObject) {
        ...
        delegate?.onCroppedImage!(image: image)
        dismiss(animated: true, completion: nil)
...
//        ProfileViewController.state = "crop"
    }
}
